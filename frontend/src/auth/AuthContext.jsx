...
export function AuthProvider({ children }) {
  ...
  const [role, setRole] = useState(() => localStorage.getItem("lhc_role") || "");
  const [userId, setUserId] = useState(() => localStorage.getItem("lhc_user") || "");

  function loginOk({ token, role, user_id }) {
    setToken(token); setRole(role); setUserId(user_id);
